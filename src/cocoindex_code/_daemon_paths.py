...
def daemon_runtime_dir() -> Path:
    ...
    Override with ``RAG4TREX_RUNTIME_DIR``. Defaults to
    :func:`user_settings_dir` for backward compatibility — non-Docker users
    see identical behavior to before the split.
    """
    override = os.environ.get("RAG4TREX_RUNTIME_DIR") or os.environ.get(
...
def daemon_socket_path() -> str:
    ...
    if sys.platform == "win32":
        ...
        # Hash the runtime dir so RAG4TREX_RUNTIME_DIR (or the RAG4TREX_DIR
        # fallback) overrides produce unique pipe names,
        ...
        return rf"\\.\pipe\rag4trex_{dir_hash}"
